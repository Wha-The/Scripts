local Glib = {Toggle = true,FirstTab = nil,TabCount = 0,ColorTable = {}}
local RunService = game:GetService("RunService")
local HttpService = game:GetService("HttpService")
local TweenService = game:GetService("TweenService")
local UserInputService = game:GetService("UserInputService")

local WindowName = HttpService:GenerateGUID(false)
...
function Glib:CreateWindow(Config, Parent)
	local WindowInit = {}
	...
	local function ResetAll()
		...
		for _,TabButton in pairs(TBContainer:GetChildren()) do
			if TabButton:IsA("TextButton") then
				TabButton.Size = UDim2.new(0,480 / Glib.TabCount,1,0)
			end
		end
	...
	local function KeepFirst()
		for _,Tab in pairs(TContainer:GetChildren()) do
			if Tab:IsA("ScrollingFrame") then
				if Tab.Name == Glib.FirstTab .. " T" then
					Tab.Visible = true
				else
					Tab.Visible = false
				end
		...
		for _,TabButton in pairs(TBContainer:GetChildren()) do
			if TabButton:IsA("TextButton") then
				if TabButton.Name == Glib.FirstTab .. " TB" then
	...
	local function ChangeColor(Color)
		Config.Color = Color
		for i, v in pairs(Glib.ColorTable) do
	...
	function WindowInit:CreateTab(Name)
		...
		table.insert(Glib.ColorTable, TabButton)
		Glib.TabCount = Glib.TabCount + 1
		if Glib.TabCount == 1 then
			Glib.FirstTab = Name
		end
		...
		function TabInit:CreateSection(Name)
			...
			function SectionInit:CreateLabel(Name, Text)
				local LabelInit = {}
			...
			function SectionInit:CreateButton(Name, Callback)
				...
				table.insert(Glib.ColorTable, Button)
			...
			function SectionInit:CreateToggle(Name, Default, Callback)
				...
				table.insert(Glib.ColorTable, Toggle.Toggle)
			...
			function SectionInit:CreateSlider(Name, Min, Max, Default, Precise, Callback)
				...
				table.insert(Glib.ColorTable, Slider.Slider.Bar)
			...
			function SectionInit:CreateDropdown(Name, OptionTable, Callback, InitialValue)
				...
				for _,OptionName in pairs(OptionTable) do
					...
					table.insert(Glib.ColorTable, Option)
