...
local api = {}
do
    api.nickname = game.Players.LocalPlayer.Name
    api.humanoidrootpart = function()
    ...
    api.findvalue = function(l, s)
        return table.find(l, s)
    end
    api.partwithnamepart = function(s, folder)
    ...
    api.tween = function(time, cframe)
        local t = game:GetService("TweenService"):Create(api.humanoidrootpart(), TweenInfo.new(time), {
            CFrame = cframe
        })
        t:Play()
        t.Completed:Wait()
    end
    api.walkTo = function(vector3)
        return api.humanoid():MoveTo(vector3)
    end
    api.tablefind = table.find
    api.teleport = function(cframe)
        api.humanoidrootpart().CFrame = cframe
    end
    api.returnvalue = function(ss, s)
        if string.find(ss, s) then
            return s
        end
    end
    api.getbiggestmodel = function(g)
        local biggest
        local bm
    ...
    api.suffixstring = function(x)
        ...
        if x < 1000 then
            ...
            visible = x / math.pow(10, index * 3)
            suffix = abbreviations[index] .. "+"
        end
        local front = visible / math.pow(10, 0)

        return string.format("%i%s", front, suffix)
    end
    api.toHMS = function(x)
